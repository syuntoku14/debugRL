...
class IpgSolver(Solver):
    def run(self, num_steps=10000):
        for _ in tqdm(range(num_steps)):
            ...
            off_trajectory = self.buffer.sample(self.solve_options["minibatch_size"])
            tensor_off_traj = utils.trajectory_to_tensor(off_trajectory, self.device)

            # ----- update networks -----
            on_actor_loss, off_actor_loss = self.update_actor(
                tensor_traj, tensor_off_traj
            )
    ...
    def compute_coef(self, traj):
        ...
        if self.is_tabular:
            ...
            self.record_scalar(
                "AdvantageError",
                np.mean(((Q - values) - (oracle_Q - oracle_V)) ** 2),
                tag="Q-V",
            )
            self.record_scalar(
                "AdvantageError", np.mean((GAE - (oracle_Q - oracle_V)) ** 2), tag="GAE"
            )
    ...
    def update_actor(self, tensor_traj, tensor_off_traj):
        ...
        off_policy = torch.softmax(self.policy_network(off_obss), dim=-1)  # BxA
        off_policy = (
            off_policy + (off_policy == 0.0).float() * 1e-8
        )  # avoid numerical instability
        values = self.value_network(off_obss).detach()  # BxA
        off_loss = (
            -self.solve_options["on_off_coef"] * ((values * off_policy).sum(1)).mean()
        )
    ...
    def update_critic(self, tensor_traj):
        discount = self.solve_options["discount"]
        obss, next_obss, act, rews, dones, timeouts = (
            tensor_traj["obs"],
        ...
        with torch.no_grad():
            next_policy = torch.softmax(self.policy_network(next_obss), dim=-1)  # BxA
        ...
        values = self.value_network(obss).gather(1, act.reshape(-1, 1)).squeeze()
    ...
    def set_tb_values_policy(self):
        ...
        preference = (
            self.policy_network(self.all_obss)
            .reshape(self.dS, self.dA)
            .detach()
            .cpu()
            .numpy()
